...
if (hasBadKey(var_export($xml,true))) {
	header("Location: http://anilot.tk/");
	exit();
} 
...
$str_page_arg = getArgDataFromStr($str_page);
$url_map_array = fmt_url_map($str_page_arg);
$qualitylist = array();
foreach ($url_map_array as $t) {
	$t_num = substr($t,0,strpos($t,'|'));
	//$t_url=substr($t,strlen($t_num)+1);
	$t_name = $QualityList[$t_num];
	array_push($qualitylist,$t_num);
}
